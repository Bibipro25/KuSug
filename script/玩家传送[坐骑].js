...
function uint64BEHex(ms) {
    const hi = Math.floor(ms / 0x100000000);
    const lo = ms - hi * 0x100000000;
    return intHex(hi, 4) + intHex(lo, 4);
}
...
function summonMount() {
    ...
    sendPyRpc(MOUNT_RPC_ID, '93c40163920681c4057374617274cf' + uint64BEHex(Date.now()) + 'c0');
    sendPyRpc(MOUNT_RPC_ID, '93c401729208c4244d696e6563726166743a7065743a74656c65706f72745f6d6f756e745f72657175657374c0');
    sendPyRpc(MOUNT_RPC_ID, '93c40163920881c408706c617965724964' + msgpackBinHex(myId) + 'c0');
    return true;
}

function snapshotNearbyIds() {
    const set = {};
    try {
        const me = player.getLocalPlayer();
        const myPos = me.getPos();
        const actors = world.getClientWorld().getActors() || [];
        for (const a of actors) {
            try {
                const p = a.getPos();
                const dx = p.x - myPos.x, dy = p.y - myPos.y, dz = p.z - myPos.z;
                if (dx * dx + dy * dy + dz * dz <= 100) set[a.getUniqueID()] = true;
            } catch (e) {}
        }
    } catch (e) {}
    return set;
...
function findNewMount(beforeSet) {
    try {
        const level = world.getClientWorld();
        const me = player.getLocalPlayer();
        const myId = me.getUniqueID();
        const myPos = me.getPos();
        const playerIds = {};
        const plist = level.getPlayerList() || {};
        for (const k in plist) {
            if (plist[k] && plist[k].id) playerIds[plist[k].id] = true;
        }
        const actors = level.getActors() || [];
        const maxDist = MOUNT_RANGE * MOUNT_RANGE;
        let best = null;
        let bestDist = maxDist;
        for (const a of actors) {
            try {
                const uid = a.getUniqueID();
                if (!uid || uid === myId || beforeSet[uid] || playerIds[uid]) continue;
                const p = a.getPos();
                const dx = p.x - myPos.x, dy = p.y - myPos.y, dz = p.z - myPos.z;
...
function tryRidePending() {
    if (!pendingRide) return;

    try {
        const me = player.getLocalPlayer();
        if (me.getVehicle()) {
            const pos = pendingRide.pos;
            const name = pendingRide.name;
            pendingRide = null;
            setTimeout(() => {
                minecraft.requestExecuteCommand(`/ww tp ${pos.x} ${pos.y} ${pos.z}`, () => {
                    info(`§a已骑乘坐骑传送至 §e${name} §7[${pos.x}, ${pos.y}, ${pos.z}]`);
                });
            }, MOUNT_DELAY_MS);
            return;
        }
        if (!pendingRide.interacted) {
            const mount = findNewMount(pendingRide.beforeSet);
            if (mount) {
                me.interact(mount);
                pendingRide.interacted = true;
            }
        }
    } catch (e) {}

    if (pendingRide && Date.now() > pendingRide.deadline) {
        info(`§c半径 ${MOUNT_RANGE} 格内未检测到可用坐骑,已取消传送`);
        pendingRide = null;
    }
}
...
function requestTeleport(targetId) {
    ...
    const beforeSet = snapshotNearbyIds();
    if (!summonMount()) {
        info('§c坐骑召唤失败,已取消传送');
        return;
    }
...
function onPyRpcReceiveEvent(id, data) {
    ...
    try {
        ...
        pendingRide = {
            beforeSet: pendingTeleport.beforeSet || {},
            pos: pos,
            name: pendingTeleport.name,
            deadline: Date.now() + MOUNT_WAIT_MS,
            interacted: false
        };
        pendingTeleport = null;
        tryRidePending();
